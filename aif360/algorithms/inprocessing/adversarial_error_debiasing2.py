...
try:
    ...
    warning("{}: AdversarialErrorDebiasing will be unavailable. To install, run:\n"
            "pip install 'aif360[AdversarialDebiasing]'".format(error))

from aif360.algorithms import Transformer


class AdversarialErrorDebiasing(Transformer):
    """Adversarial error debiasing is an in-processing technique that learns a
    classifier to maximize prediction accuracy and simultaneously reduce an
    adversary's ability to determine the protected attribute from the
    error. This approach leads to a fair classifier as the
    predictions cannot carry any group discrimination information that the
    adversary can exploit. Builds on [5].
    ...
    def _adversary_model(self, pred_logits, true_labels, true_protected):
        """Compute the adversary predictions for the protected attribute.
        """
        with tf.variable_scope("adversary_model"):
            c = tf.get_variable('c', initializer=tf.constant(1.0))
            s = tf.sigmoid((1 + tf.abs(c)) * true_protected)

            W2 = tf.get_variable('W2', [3, 1],
                                 initializer=tf.initializers.glorot_uniform(seed=self.seed4))
            b2 = tf.Variable(tf.zeros(shape=[1]), name='b2')

            pred_protected_attribute_logit = tf.matmul(tf.concat([s, s,s]), W2) + b2
            pred_protected_attribute_label = tf.nn.relu(pred_protected_attribute_logit)
    ...
    def fit(self, dataset):
        ...
        if tf.executing_eagerly():
            raise RuntimeError("AdversarialErrorDebiasing does not work in eager "
        ...
        with tf.variable_scope(self.scope_name):
            ...
            if self.debias:
                ...
                model_errors = tf.abs(pred_logits - self.true_labels_ph)
                pred_protected_attributes_labels, pred_protected_attributes_logits = self._adversary_model(pred_logits, self.true_labels_ph, self.protected_attributes_ph)
                pred_protected_attributes_loss = tf.reduce_mean(
                    tf.squared_difference(model_errors, pred_predicted_attribte_label))
